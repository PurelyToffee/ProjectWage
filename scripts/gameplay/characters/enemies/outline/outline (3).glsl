...
void main() {
	...
	// Scene depth at this pixel (reversed-Z: 1.0 = near, 0.0 = far)
	float scene_depth = texture(depth_texture, uv).r;

	int thickness = int(params.thickness);
	float max_neighbor_mask = 0.0;
	float max_neighbor_depth = 0.0; // highest = closest to camera in reversed-Z
	...
	for (int x = -thickness; x <= thickness; x++) {
		for (int y = -thickness; y <= thickness; y++) {
			...
			if (neighbor_mask > 0.5) {
				...
				// In reversed-Z, closer = higher value
	...
	// Occluded if something at this pixel is CLOSER (higher depth) than the enemy
	bool occluded = scene_depth > max_neighbor_depth + 0.0001;
