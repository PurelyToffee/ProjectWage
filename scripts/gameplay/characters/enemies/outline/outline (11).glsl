#[compute]
#version 450

layout(local_size_x = 8, local_size_y = 8, local_size_z = 1) in;

layout(set = 0, binding = 0, std430) readonly buffer Params {
	vec2 raster_size;
	vec2 reserved;
	vec4 outline_color;
	float thickness;
	float reserved2;
	float reserved3;
	float reserved4;
} params;

layout(rgba16f, set = 0, binding = 1) uniform image2D color_image;
layout(set = 0, binding = 2) uniform sampler2D depth_texture;
layout(set = 0, binding = 3) uniform sampler2D mask_texture;
layout(set = 0, binding = 4) uniform sampler2D enemy_depth_texture;

void main() {
	ivec2 coord = ivec2(gl_GlobalInvocationID.xy);
	vec2 size = params.raster_size;

	if (float(coord.x) >= size.x || float(coord.y) >= size.y)
		return;

	vec2 uv = (vec2(coord) + 0.5) / size;

	if (texture(mask_texture, uv).a > 0.5)
		return;

	// Scene depth at this pixel (layer 1: world + enemies)
	float scene_depth = texture(depth_texture, uv).r;

	// Find if this is an outline pixel
	int thickness = int(params.thickness);
	bool is_outline = false;
	float max_enemy_depth = 0.0; // reversed-Z: track closest (highest) enemy neighbor depth

	for (int x = -thickness; x <= thickness; x++) {
		for (int y = -thickness; y <= thickness; y++) {
			if (length(vec2(x, y)) > float(thickness) + 0.5)
				continue;
			vec2 offset_uv = uv + vec2(x, y) / size;
			if (texture(mask_texture, offset_uv).a > 0.5) {
				is_outline = true;
				// Use enemy depth AT THE OUTLINE PIXEL (uv), not at the neighbor
				// This way we compare depths at the same screen position
				float ed = texture(enemy_depth_texture, uv).r;
				if (ed > max_enemy_depth)
					max_enemy_depth = ed;
			}
		}
	}

	if (!is_outline)
		return;

	// If enemy depth texture has nothing at this pixel, fall back to neighbor depths
	if (max_enemy_depth < 0.0001) {
		for (int x = -thickness; x <= thickness; x++) {
			for (int y = -thickness; y <= thickness; y++) {
				if (length(vec2(x, y)) > float(thickness) + 0.5)
					continue;
				vec2 offset_uv = uv + vec2(x, y) / size;
				if (texture(mask_texture, offset_uv).a > 0.5) {
					float ed = texture(enemy_depth_texture, offset_uv).r;
					if (ed > max_enemy_depth)
						max_enemy_depth = ed;
				}
			}
		}
	}

	// Reversed-Z: scene_depth > enemy_depth means scene geometry is closer → occlude
	if (scene_depth > max_enemy_depth + 0.0001)
		return;

	vec4 current = imageLoad(color_image, coord);
	float alpha = params.outline_color.a;
	imageStore(color_image, coord, vec4(mix(current.rgb, params.outline_color.rgb, alpha), current.a));
}
