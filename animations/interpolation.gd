...
func _run():   # this is the main function
	...
	if selection.size()!=1 and not selection is AnimationPlayer: # if the wrong node is selected, do nothing
		...
		interpolation_change(selection[0]) # run the funstion in question



func interpolation_change(selection : AnimationPlayer):
	print(selection.get_animation_list())
	
	var tracks = selection.get_animation_list();
	
	for t in tracks:
		var anim_track_1 = selection.get_animation(t) # get the Animation that you are interested in (change "default" to your Animation's name)
